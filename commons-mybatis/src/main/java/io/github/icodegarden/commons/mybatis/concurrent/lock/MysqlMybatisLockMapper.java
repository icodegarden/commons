...
public interface MysqlMybatisLockMapper extends DatabaseLockDao {
	 ...
	 * 获取处于锁中的identifier
	 */
	@Select("<script> select identifier from " + DatabaseLockDao.TABLE_NAME
	 ...
	 * 锁数据是否存在
	 */
	@Select("<script> select id from " + DatabaseLockDao.TABLE_NAME + " where name = #{lockName}</script>")
	@Override
	Long findRow(@Param("lockName") String lockName);

	@Insert("<script> insert into " + DatabaseLockDao.TABLE_NAME
	...
	@Update("<script> update " + DatabaseLockDao.TABLE_NAME
	...
	@Update("<script> update " + DatabaseLockDao.TABLE_NAME + " set is_locked=0 where name=#{lockName}</script>")
	@Override
